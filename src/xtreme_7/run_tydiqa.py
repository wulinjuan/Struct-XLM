...
def train(args, train_dataset, model, actorModel, tokenizer):
    """ Train the model """
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:
                        results = evaluate(args, model, actorModel, tokenizer)
...
def evaluate(args, model, actorModel, tokenizer, prefix="", language='en', lang2id=None, dataset_name='squad'):
...
def main():
    ...
    for param in actorModel.target_policy.parameters():
        mulValue = np.prod(param.size())  # 使用numpy prod接口计算参数数组所有元素之积
        Total_params += mulValue  # 总参数量
        if param.requires_grad:
            Trainable_params += mulValue  # 可训练参数量
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(args, train_dataset, model, actorModel, tokenizer)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            # Evaluate
            result = evaluate(args, model, actorModel, tokenizer, prefix=global_step, language=args.eval_lang,
                              lang2id=lang2id)
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        if "mlqa" in args.test_file:
            ...
            for lang in ["ar", "de", "en", "es", "hi", "vi", "zh"]:
                ...
                result = evaluate(args, model, actorModel, tokenizer, prefix=u"mlqa {}".format(lang), language=lang,
        ...
        if "tydiqa" in args.test_file:
            result_str = ""
            result_file = open("output/tydiqa_results.txt", "a+", encoding='utf-8')
            f1_all = 0
            em_all = 0
